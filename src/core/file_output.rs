...
impl FilesOutput {
    pub fn run(&mut self, postions: &Vec<Position>, pair_mem: &PairMemory, money: &Money) {
        println!("web {:?} ...", &pair_mem.pair);
        self.write_trend_analyse_output(&pair_mem.sky_eng, &postions);
        self.write_web_output(&pair_mem.sky_eng, &postions, self.cfg.days_out);
