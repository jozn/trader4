...
impl BTickData {
    ...
    fn from_csv_old(csv_row: csv::StringRecord) -> Self {
        use std::convert::*;
...
pub fn load_rows_dep(file_path: &str) -> Vec<BTickData> {
    let mut arr = Vec::new();
    ...
    for result in rdr.records() {
        match result {
            Ok(csv_row) => {
                ...
                let row = BTickData::from_csv_old(csv_row);
                // let row =
                let r = arr.push(row);
                // println!("err :{:?}", r );
...
pub fn load_all_pair_dep(pair: &Pair, rng: Range<u16>) -> Vec<BTickData> {
    let mut arr = vec![];
...
pub fn load_rows_fast(file_path: &str) -> Vec<BTickData> {
    ...
    for b in arr_bins {
        out_arr.push(b.to_tick(&Pair::EURUSD));
    }
    // println!("num :{}", i);
