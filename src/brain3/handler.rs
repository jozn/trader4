...
impl Brain3 {
    ...
    pub fn on_price_tick22(&mut self, symbol_id: i64, tick: Tick) {
        ...
        match frame_opt {
            ...
            Some(frame) => {
                ...
                if dc_str.dis_bull == 2 && frame.trd2 > 0. && frame.vel.avg_vel_pip > 0. {
                    self.go_long(1, frame.fid, &tick, &frame);
                    // self.go_short(1, frame.fid, &tick, &frame);
                }

                if dc_str.dis_bear == 2 && frame.trd2 < 0. && frame.vel.avg_vel_pip < 0. {
                    self.go_short(1, frame.fid, &tick, &frame);
                    // self.go_long(1, frame.fid, &tick, &frame);
                }
            }
    ...
    pub fn on_price_tick_bk2(&mut self, symbol_id: i64, tick: Tick) {
        self.last_tick = Some(tick.clone());
